...
const ConsentModal = ({ updateConsent, setShowModal }: ModalProps) => {
  ...
  useEffect(() => {
    console.log("Consent is:", consent); // Log current consent value
    if (consent === null) {
      console.log("Setting modal visible"); // Check if this runs
  ...
  if (!isVisible || consent !== null) {
    console.log("Modal not visible due to:", isVisible, consent); // Why isn't the modal visible?
    return null;
  }
